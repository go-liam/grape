...
var JL_pgShPicClass = function()
{
   ...
   this.DelCallBack =function(){
	 ...
	 this.AddEvAj =function(eid){
		var purl= cb.V("aa_url") ;
		if( cb.VL2(purl)<2 ){
			jlb.FromAlert("请选择图片");
			return false ;
		}
		if(regVars.isDoing) return ;
		var gourl= $("#go_url").val();
		//var ty= $("#tyid").val(); 
		var oder= $("#aa_OrderID").val(); 
		regVars.isDoing = true ;
	    var pur= "ad_picact.aspx?act=add&i="+eid  ;
		 $.post(pur,
		{
		  g:gourl ,
		  u:purl ,
	 ...
	 this.EditEv =function(i){
		...
		//	return false ;
		//}
		var aa_url =$("#aa_url"+i).val(); 
		var go_url = $("#go_url"+i).val(); 
		if(regVars.isDoing) return ;
		var oder=$("#aa_OrderID"+i).val(); 
		regVars.isDoing = true ;
	    var pur= "ad_picact.aspx?act=edit&i="+i ;
		 $.post(pur,
		{
		  gu:go_url ,
		  u:aa_url ,
	...
	this.ListActEv=function(){
		  ...
		  if(a==""){
		   ...
		   var u= "ad_picact.aspx?jsonp=jlpcl.ListActCallBack&"+PassUrl+"&ttt="+cb.time();
		  cb.newScript( u);
		  regVars.isDoing = true ;
