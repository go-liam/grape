...
const (
	sqlFindOne = "select .+ from sa_server"
)
...
func TestSqlServer_Create(t *testing.T) {
	db, mock, _ := mysql.MockEngine(mysql.ServerAPI) //getMockDB(t)
	defer db.Close()
	sql := "INSERT INTO `sa_server`" // regexp.QuoteMeta("SELECT * from cp_admin")
...
func TestDao_FindMulti(t *testing.T) {
	...
	sqlCount := regexp.QuoteMeta("SELECT count(*) FROM `sa_server`")
...
func TestDao_FindMultiByNil(t *testing.T) {
	db, mock, _ := mysql.MockEngine(mysql.ServerAPI) //getMockDB(t)
	defer db.Close()
	sql := sqlFindOne // regexp.QuoteMeta("SELECT * from cp_admin")
