...
func SetupRouterMetric(engine *gin.Engine) {
	g := engine.Group(core.Path)
	//链路跟踪
	tracing.Init(trace.DefaultConfig)
	engine.Use(tracing.NewGinMiddlewareHandle(core.ServerName))
	// 限流
	...
	if metric.Server.IsOpen {
		...
		engine.GET(core.Path+"/metrics/v2", gin.WrapH(metric.Server.Handler()))
	} else {
		engine.GET(core.Path+"/metrics/v2", metric.Server.HandlerClose)
	}
